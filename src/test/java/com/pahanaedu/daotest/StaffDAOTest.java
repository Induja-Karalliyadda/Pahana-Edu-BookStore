...
class StaffDAOTest {
  ...
  void requireDatabase() throws Exception {
    // Skip if DB isn't available
    try (Connection c = Db.get().getConnection()) {
      assumeTrue(c != null && !c.isClosed(), "DB not available for StaffDAO tests");
    } catch (Throwable t) {
  ...
  void cleanup() {
    if (createdId != null) {
      try { dao.delete(createdId); } catch (Exception ignore) {}
      createdId = null;
    }
  }

  @Test
  void insert_findAll_update_delete_roundtrip() {
    ...
    s.setEmail(email1);
    s.setPassword("p@ssw0rd"); // hashed by SQL SHA2(?,256)

    assertTrue(dao.insert(s), "Insert should succeed");

    createdId = findIdByUsername(uniq);
    assertNotNull(createdId, "Inserted staff id should be discoverable via findAll()");
    ...
    // password not set -> should use no-password SQL path
    assertTrue(dao.update(upd), "Update (no password) should succeed");
    ...
    assertTrue(dao.update(upd), "Update (with password) should succeed");
    // We can't read hashed password via DAO; just ensure other fields remain
    ...
    // ---- delete ----
    assertTrue(dao.delete(createdId), "Delete should return true");
    assertFalse(findById(createdId).isPresent(), "Deleted staff should not be returned by findAll()");
  ...
  void delete_nonExisting_returnsFalse() {
    assertFalse(dao.delete(-98765));
  }

  // ---------- helpers using DAO.findAll() ----------
